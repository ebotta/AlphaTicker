...
# get today's date and calculate the date 365 days ago for API queries
date_today = str(datetime.today().date())
result_date_string = subtract_1yr_from_date_string(date_today)
...
for ticker in CN_tickers:
    ...
    if int(j) > 50:
        ...
        if avg_volume > 100000:
            ...
            day360_date = dates[247]  # Assuming the last date in the data is the most recent date            
            ...
            net30_volume = ((volume30 - volume0) / volume0 if volume0 else None) - ((GDX_volume30 - GDX_volume0) / GDX_volume0 if GDX_volume0 else None)
            ...
            if zscore30_price is not None and zscore30_price > threshold:
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    ...
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '30-day Return', 'GDX 30-day Return', 'Net 30-day Return Delta', 'Z-score of Net 30-day Return Delta'])
                    alpha_writer.writerow([ticker, day30_date, f"{close30_return:.2%}" if close30_return is not None else "N/A", f"{GDX_close30_return:.2%}" if GDX_close30_return is not None else "N/A", f"{net30_close:.2%}" if net30_close is not None else "N/A", f"{zscore30_price:.2f}" if zscore30_price is not None else "N/A"])
            if zscore30_volume is not None and zscore30_volume > threshold:
                ...
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    ...
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '30-day Volume Return', 'GDX 30-day Volume Return', 'Net 30-day Volume Return Delta', 'Z-score of Net 30-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day30_date, f"{(volume30 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume30 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net30_volume:.2%}" if net30_volume is not None else "N/A", f"{zscore30_volume:.2f}" if zscore30_volume is not None else "N/A"])
            ...
            net60_volume = ((volume60 - volume0) / volume0 if volume0 else None) - ((GDX_volume60 - GDX_volume0) / GDX_volume0 if GDX_volume0 else None)
            ...
            if zscore60_price is not None and zscore60_price > threshold:
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    alpha_writer = csv.writer(alpha_file)
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '60-day Return', 'GDX 60-day Return', 'Net 60-day Return Delta', 'Z-score of Net 60-day Return Delta'])
                    alpha_writer.writerow([ticker, day60_date, f"{close60_return:.2%}" if close60_return is not None else "N/A", f"{GDX_close60_return:.2%}" if GDX_close60_return is not None else "N/A", f"{net60_close:.2%}" if net60_close is not None else "N/A", f"{zscore60_price:.2f}" if zscore60_price is not None else "N/A"])
            if zscore60_volume is not None and zscore60_volume > threshold:
                write_mode = 'a' if os.path.exists('Alpha_Volume.csv') else 'w'
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    alpha_volume_writer = csv.writer(alpha_volume_file)
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '60-day Volume Return', 'GDX 60-day Volume Return', 'Net 60-day Volume Return Delta', 'Z-score of Net 60-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day60_date, f"{(volume60 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume60 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net60_volume:.2%}" if net60_volume is not None else "N/A", f"{zscore60_volume:.2f}" if zscore60_volume is not None else "N/A"])
            ...
            if zscore90_price is not None and zscore90_price > threshold:
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    alpha_writer = csv.writer(alpha_file)
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '90-day Return', 'GDX 90-day Return', 'Net 90-day Return Delta', 'Z-score of Net 90-day Return Delta'])
                    alpha_writer.writerow([ticker, day90_date, f"{close90_return:.2%}" if close90_return is not None else "N/A", f"{GDX_close90_return:.2%}" if GDX_close90_return is not None else "N/A", f"{net90_close:.2%}" if net90_close is not None else "N/A", f"{zscore90_price:.2f}" if zscore90_price is not None else "N/A"])
            if zscore90_volume is not None and zscore90_volume > threshold:
                write_mode = 'a' if os.path.exists('Alpha_Volume.csv') else 'w'
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    alpha_volume_writer = csv.writer(alpha_volume_file)
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '90-day Volume Return', 'GDX 90-day Volume Return', 'Net 90-day Volume Return Delta', 'Z-score of Net 90-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day90_date, f"{(volume90 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume90 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net90_volume:.2%}" if net90_volume is not None else "N/A", f"{zscore90_volume:.2f}" if zscore90_volume is not None else "N/A"])
            ...
            if zscore120_price is not None and zscore120_price > threshold:
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    alpha_writer = csv.writer(alpha_file)
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '120-day Return', 'GDX 120-day Return', 'Net 120-day Return Delta', 'Z-score of Net 120-day Return Delta'])
                    alpha_writer.writerow([ticker, day120_date, f"{close120_return:.2%}" if close120_return is not None else "N/A", f"{GDX_close120_return:.2%}" if GDX_close120_return is not None else "N/A", f"{net120_close:.2%}" if net120_close is not None else "N/A", f"{zscore120_price:.2f}" if zscore120_price is not None else "N/A"])
            if zscore120_volume is not None and zscore120_volume > threshold:
                write_mode = 'a' if os.path.exists('Alpha_Volume.csv') else 'w'
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    alpha_volume_writer = csv.writer(alpha_volume_file)
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '120-day Volume Return', 'GDX 120-day Volume Return', 'Net 120-day Volume Return Delta', 'Z-score of Net 120-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day120_date, f"{(volume120 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume120 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net120_volume:.2%}" if net120_volume is not None else "N/A", f"{zscore120_volume:.2f}" if zscore120_volume is not None else "N/A"])
            ...
            if zscore150_price is not None and zscore150_price > threshold:
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    alpha_writer = csv.writer(alpha_file)
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '150-day Return', 'GDX 150-day Return', 'Net 150-day Return Delta', 'Z-score of Net 150-day Return Delta'])
                    alpha_writer.writerow([ticker, day150_date, f"{close150_return:.2%}" if close150_return is not None else "N/A", f"{GDX_close150_return:.2%}" if GDX_close150_return is not None else "N/A", f"{net150_close:.2%}" if net150_close is not None else "N/A", f"{zscore150_price:.2f}" if zscore150_price is not None else "N/A"])
            if zscore150_volume is not None and zscore150_volume > threshold:
                write_mode = 'a' if os.path.exists('Alpha_Volume.csv') else 'w'
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    alpha_volume_writer = csv.writer(alpha_volume_file)
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '150-day Volume Return', 'GDX 150-day Volume Return', 'Net 150-day Volume Return Delta', 'Z-score of Net 150-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day150_date, f"{(volume150 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume150 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net150_volume:.2%}" if net150_volume is not None else "N/A", f"{zscore150_volume:.2f}" if zscore150_volume is not None else "N/A"])
            ...
            if zscore180_price is not None and zscore180_price > threshold:
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    alpha_writer = csv.writer(alpha_file)
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '180-day Return', 'GDX 180-day Return', 'Net 180-day Return Delta', 'Z-score of Net 180-day Return Delta'])
                    alpha_writer.writerow([ticker, day180_date, f"{close180_return:.2%}" if close180_return is not None else "N/A", f"{GDX_close180_return:.2%}" if GDX_close180_return is not None else "N/A", f"{net180_close:.2%}" if net180_close is not None else "N/A", f"{zscore180_price:.2f}" if zscore180_price is not None else "N/A"])
            if zscore180_volume is not None and zscore180_volume > threshold:
                write_mode = 'a' if os.path.exists('Alpha_Volume.csv') else 'w'
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    alpha_volume_writer = csv.writer(alpha_volume_file)
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '180-day Volume Return', 'GDX 180-day Volume Return', 'Net 180-day Volume Return Delta', 'Z-score of Net 180-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day180_date, f"{(volume180 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume180 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net180_volume:.2%}" if net180_volume is not None else "N/A", f"{zscore180_volume:.2f}" if zscore180_volume is not None else "N/A"])
            ...
            if zscore210_price is not None and zscore210_price > threshold:
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    alpha_writer = csv.writer(alpha_file)
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '210-day Return', 'GDX 210-day Return', 'Net 210-day Return Delta', 'Z-score of Net 210-day Return Delta'])
                    alpha_writer.writerow([ticker, day210_date, f"{close210_return:.2%}" if close210_return is not None else "N/A", f"{GDX_close210_return:.2%}" if GDX_close210_return is not None else "N/A", f"{net210_close:.2%}" if net210_close is not None else "N/A", f"{zscore210_price:.2f}" if zscore210_price is not None else "N/A"])
            if zscore210_volume is not None and zscore210_volume > threshold:
                write_mode = 'a' if os.path.exists('Alpha_Volume.csv') else 'w'
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    alpha_volume_writer = csv.writer(alpha_volume_file)
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '210-day Volume Return', 'GDX 210-day Volume Return', 'Net 210-day Volume Return Delta', 'Z-score of Net 210-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day210_date, f"{(volume210 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume210 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net210_volume:.2%}" if net210_volume is not None else "N/A", f"{zscore210_volume:.2f}" if zscore210_volume is not None else "N/A"])
            ...
            if zscore240_price is not None and zscore240_price > threshold:
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    alpha_writer = csv.writer(alpha_file)
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '240-day Return', 'GDX 240-day Return', 'Net 240-day Return Delta', 'Z-score of Net 240-day Return Delta'])
                    alpha_writer.writerow([ticker, day240_date, f"{close240_return:.2%}" if close240_return is not None else "N/A", f"{GDX_close240_return:.2%}" if GDX_close240_return is not None else "N/A", f"{net240_close:.2%}" if net240_close is not None else "N/A", f"{zscore240_price:.2f}" if zscore240_price is not None else "N/A"])
            if zscore240_volume is not None and zscore240_volume > threshold:
                write_mode = 'a' if os.path.exists('Alpha_Volume.csv') else 'w'
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    alpha_volume_writer = csv.writer(alpha_volume_file)
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '240-day Volume Return', 'GDX 240-day Volume Return', 'Net 240-day Volume Return Delta', 'Z-score of Net 240-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day240_date, f"{(volume240 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume240 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net240_volume:.2%}" if net240_volume is not None else "N/A", f"{zscore240_volume:.2f}" if zscore240_volume is not None else "N/A"])   
            ...
            if zscore270_price is not None and zscore270_price > threshold:
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    alpha_writer = csv.writer(alpha_file)
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '270-day Return', 'GDX 270-day Return', 'Net 270-day Return Delta', 'Z-score of Net 270-day Return Delta'])
                    alpha_writer.writerow([ticker, day270_date, f"{close270_return:.2%}" if close270_return is not None else "N/A", f"{GDX_close270_return:.2%}" if GDX_close270_return is not None else "N/A", f"{net270_close:.2%}" if net270_close is not None else "N/A", f"{zscore270_price:.2f}" if zscore270_price is not None else "N/A"])
            if zscore270_volume is not None and zscore270_volume > threshold:
                write_mode = 'a' if os.path.exists('Alpha_Volume.csv') else 'w'
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    alpha_volume_writer = csv.writer(alpha_volume_file)
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '270-day Volume Return', 'GDX 270-day Volume Return', 'Net 270-day Volume Return Delta', 'Z-score of Net 270-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day270_date, f"{(volume270 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume270 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net270_volume:.2%}" if net270_volume is not None else "N/A", f"{zscore270_volume:.2f}" if zscore270_volume is not None else "N/A"])   
            ...
            if zscore300_price is not None and zscore300_price > threshold:
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    alpha_writer = csv.writer(alpha_file)
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '300-day Return', 'GDX 300-day Return', 'Net 300-day Return Delta', 'Z-score of Net 300-day Return Delta'])
                    alpha_writer.writerow([ticker, day300_date, f"{close300_return:.2%}" if close300_return is not None else "N/A", f"{GDX_close300_return:.2%}" if GDX_close300_return is not None else "N/A", f"{net300_close:.2%}" if net300_close is not None else "N/A", f"{zscore300_price:.2f}" if zscore300_price is not None else "N/A"])
            if zscore300_volume is not None and zscore300_volume > threshold:
                write_mode = 'a' if os.path.exists('Alpha_Volume.csv') else 'w'
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    alpha_volume_writer = csv.writer(alpha_volume_file)
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '300-day Volume Return', 'GDX 300-day Volume Return', 'Net 300-day Volume Return Delta', 'Z-score of Net 300-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day300_date, f"{(volume300 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume300 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net300_volume:.2%}" if net300_volume is not None else "N/A", f"{zscore300_volume:.2f}" if zscore300_volume is not None else "N/A"])   
            ...
            if zscore330_price is not None and zscore330_price > threshold: 
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    alpha_writer = csv.writer(alpha_file)
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '330-day Return', 'GDX 330-day Return', 'Net 330-day Return Delta', 'Z-score of Net 330-day Return Delta'])
                    alpha_writer.writerow([ticker, day330_date, f"{close330_return:.2%}" if close330_return is not None else "N/A", f"{GDX_close330_return:.2%}" if GDX_close330_return is not None else "N/A", f"{net330_close:.2%}" if net330_close is not None else "N/A", f"{zscore330_price:.2f}" if zscore330_price is not None else "N/A"])
            if zscore330_volume is not None and zscore330_volume > threshold:
                write_mode = 'a' if os.path.exists('Alpha_Volume.csv') else 'w'
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    alpha_volume_writer = csv.writer(alpha_volume_file)
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '330-day Volume Return', 'GDX 330-day Volume Return', 'Net 330-day Volume Return Delta', 'Z-score of Net 330-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day330_date, f"{(volume330 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume330 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net330_volume:.2%}" if net330_volume is not None else "N/A", f"{zscore330_volume:.2f}" if zscore330_volume is not None else "N/A"])   
            ...
            if zscore360_price is not None and zscore360_price > threshold:
                ...
                with open('Alphas.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_file:
                    alpha_writer = csv.writer(alpha_file)
                    if write_mode == 'w':
                        alpha_writer.writerow(['Ticker', 'Date', '360-day Return', 'GDX 360-day Return', 'Net 360-day Return Delta', 'Z-score of Net 360-day Return Delta'])
                    alpha_writer.writerow([ticker, day360_date, f"{close360_return:.2%}" if close360_return is not None else "N/A", f"{GDX_close360_return:.2%}" if GDX_close360_return is not None else "N/A", f"{net360_close:.2%}" if net360_close is not None else "N/A", f"{zscore360_price:.2f}" if zscore360_price is not None else "N/A"])
            if zscore360_volume is not None and zscore360_volume > threshold:
                write_mode = 'a' if os.path.exists('Alpha_Volume.csv') else 'w'
                with open('Alpha_Volume.csv', mode=write_mode, newline='', encoding='utf-8') as alpha_volume_file:
                    alpha_volume_writer = csv.writer(alpha_volume_file)
                    if write_mode == 'w':
                        alpha_volume_writer.writerow(['Ticker', 'Date', '360-day Volume Return', 'GDX 360-day Volume Return', 'Net 360-day Volume Return Delta', 'Z-score of Net 360-day Volume Return Delta'])
                    alpha_volume_writer.writerow([ticker, day360_date, f"{(volume360 - volume0) / volume0:.2%}" if volume0 else "N/A", f"{(GDX_volume360 - GDX_volume0) / GDX_volume0:.2%}" if GDX_volume0 else "N/A", f"{net360_volume:.2%}" if net360_volume is not None else "N/A", f"{zscore360_volume:.2f}" if zscore360_volume is not None else "N/A"])   
